...
_required_fields = {
    ...
    "samples": ["df_samples_path", "reverse_reads", "kit", "stranded"],
...
class Analysis:
        ...
        for run_id in self.run_ids:
            if not (self.incoming / run_id).exists():
                raise FileNotFoundError(
                    f"Folder {run_id} not present in '/project/incoming'."
                )
                # TODO automatically pulliong the data from rose/incoming ...
        # check samples
        if not Path(self.samples["df_samples_path"]).exists():
            raise FileNotFoundError(
    ...
    def sample_df(self) -> DataFrame:
        ...
        """
        df_samples = pd.read_csv(self.samples["df_samples_path"], sep="\t")
        return df_samples
    ...
    def verify_samples(self):
        ...
        if len(not_present) > 0:
            raise ValueError(f"The samples table {self.samples['df_samples_path']} does not contain the following required columns {not_present}.")
        group_columns = []
        ...
        if len(group_columns) == 0:
            raise ValueError(f"No column starting with 'group' in {self.samples['df_samples_path']}. This is needed to define groups for comparisons.")
        for col in group_columns:
            fpath = (self.incoming / f"{col}.tsv")
            if not fpath.exists():
                raise FileNotFoundError("Group column {col} specified, but no file {str(fpath)} found.")
            ...
            if len(not_present) > 0:
                raise ValueError(f"The groups table {str(fpath)} does not contain the following required columns {not_present}.")
    ...
    def comparison_method(self, method: str) -> Any:
        ...
        options = {
            ...
            "include_other_samples_for_variance": self.comparison[method].get("include_other_samples_for_variance", True),
        }
